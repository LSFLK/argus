...
| --- | --- | --- | --- |
| `ARGUS_API_KEY` | **Yes** | - | A high-entropy API key required for all API write operations (`X-API-Key` or `Authorization: Bearer <key>`). Argus fails closed if this is missing. `ARGUS_AUTH_TOKEN` is still accepted as a fallback. |
