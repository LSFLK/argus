...
func TestAuditLog_Validate_WithConfig(t *testing.T) {
	// Import testutil to use SetupTestEnums
	// Note: We can't import testutil here due to circular dependency
	// So we'll set up enums directly
...
func TestAuditLog_Validate_EdgeCases(t *testing.T) {
	enums := &config.AuditEnums{
		EventTypes:   []string{"MANAGEMENT_EVENT", "USER_MANAGEMENT"},
		EventActions: []string{"CREATE", "READ", "UPDATE", "DELETE"},
		ActorTypes:   []string{"SERVICE", "ADMIN", "MEMBER", "SYSTEM"},
