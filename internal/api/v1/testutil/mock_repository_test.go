...
func TestMockRepository_GetAuditLogsByTraceID(t *testing.T) {
	...
	eventType1 := "MANAGEMENT_EVENT"
	eventType2 := "USER_MANAGEMENT"
	eventType3 := "DATA_FETCH"
	eventAction1 := "CREATE"
	...
	log4 := &v1models.AuditLog{
		...
		Status:     v1models.StatusSuccess,
		EventType:  eventType1,
		Action:     eventAction1, // Added Action field
		ActorType:  "SERVICE",
		ActorID:    "service-a",
		TargetType: "SERVICE",
		TargetID:   nil, // Added TargetID field
	}
...
func TestMockRepository_GetAuditLogs_Filtering(t *testing.T) {
	...
	t.Run("FilterByAction", func(t *testing.T) {
		...
		result, total, err := mockRepo.GetAuditLogs(ctx, filters)
		require.NoError(t, err)
		assert.Equal(t, int64(3), total, "Should find 3 logs with Action1")
		assert.Len(t, result, 3, "Should return 3 logs")
		for _, log := range result {
			assert.Equal(t, eventAction1, log.Action)
		}
	})
